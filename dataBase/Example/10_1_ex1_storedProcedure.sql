...
2. 스토어드 프로시저의 장점
	- 하나의 요청으로 여러 SQL문을 실행하여, 네트워크에 대한 부하 감소
        (PHP단위에서 요청을 줄여줌 - DB안에 있는 설정된 문을 프로시저로 한번에 요청해서 결과물을 보내줌)
	- 미리 구문 분석 및 내부 중간 코드로 변환을 끝내야 하므로 처리 시간이 감소
	- 데이터베이스 트리거와 결합하여 복잡한 규칙에 의한 데이터의 참조무결성 유지가 가능
        (트리거 : select를 하는 중간 또는 끝난후 추가로 작업을 자동으로 하도록 하는 기능)

3. 스토어드 프로시저의 단점
	- 사양 변경 시 외부 응용 프로그램과 할께 프로시저의 정의 변경이 필요(라인이 길어 질수록 디버깅이 힘들어 진다.)
	- 코드 자산으로서의 재사용성이 매우 비효율적(데이터베이스 구조 자체가 바뀜으로 재 생산성이 안된다.)
...
ex)DELIMITER $$
    ...
    CALL test_proc(10);       
= in_num 는 call 에서 불러오고자 하는 레코드의 수량을 의미.
