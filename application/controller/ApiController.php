...
class ApiController extends Controller{ //(3)
    ...
    public function userdlPost(){   //
        ...
        $arrReqData = json_decode($reqData, true);
// fwrite(fopen("C:/Apache24/htdocs/debug.txt", 'a'), $reqData . "\n"); // TODO: Debug용 로그 파일 생성
        $arrReqData["u_id"] = $_SESSION[_STR_LOGIN_ID];
        $arrData = [ "flg" => "0"];
        // model 호출
        $this->model = $this->getModel("User"); //model을 지정해서 사용하는 구문(지금은 user모델)

        $result = $this->model->getUser($arrReqData); // user모델을 getuser로 다시 지정해서 사용
    ...
    public function delGet(){   //
        ...
        if($result){
            ...
            $arrData["msg"] = "탈퇴처리를 완료했습니다."; 
            http_response_code(200); //각 오류 확인 값을 설정

            session_unset(); // 세션 정보 삭제
            session_destroy(); // 세션 파기
        } else{
            $arrData["errflg"] = "1";
            $arrData["msg"] = "탈퇴처리에 실패했습니다."; 
